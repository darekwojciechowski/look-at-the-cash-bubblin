...
class TestDataExportIntegration:
    ...
    def test_export_misc_to_file(
        ...
        Given: a DataFrame with two MISC rows
        When:  export_unassigned_transactions_to_csv() is called
        Then:  the expected output file exists with two rows, all with category MISC
        """
        # Arrange
    ...
    def test_google_sheets_export_format(
        ...
        Given: a processed DataFrame with expense categories and amounts
        When:  it is exported via export_for_google_sheets() and re-read
        Then:  columns match the Google Sheets schema and no NaN values are present
        """
        # Arrange
        monkeypatch.chdir(tmp_path)

        # Act
        output_file = export_for_google_sheets(sample_dataframe_with_categories)

        # Assert
        result_df = pd.read_csv(output_file, sep="\t")
        assert list(result_df.columns) == ["Txn_Id", "Day", "Month", "Year", "Item", "Category", "Amount", "Importance"]
        assert len(result_df) == len(sample_dataframe_with_categories)
        assert result_df["Txn_Id"].isna().all()  # fixture has no txn_id, exporter emits empty IDs
        non_id_columns = ["Day", "Month", "Year", "Item", "Category", "Amount", "Importance"]
        assert result_df[non_id_columns].isna().sum().sum() == 0
