...
_THREE_TOKEN_NUMBER_RE = re.compile(r"\w+\s+\w+\s+\d+")  # Pattern like "street name 123"
...
def create_google_maps_link(location: str | None) -> str:
    ...
    has_street_indicator = any(token in lowered for token in _MAPS_STREET_TOKENS)
...
def _split_parts(data_string: str | float | None) -> list[str]:
    ...
    """
    return [part.strip() for part in str(data_string).split("//") if part and part.strip()]
...
def _extract_address_payload(payload: str) -> str | None:
    ...
    without_country = re.split(r"(?i)kraj\s*:\s*", payload, maxsplit=1)[0].strip(" ,")
